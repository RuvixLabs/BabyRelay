...
- **Crash/support/attribution**: Crashlytics and FCM init with Firebase,
  ...
  plus RevenueCat identity bridging controlled by `APPREFER_API_KEY`.
...
|---|---|---|
| Firebase | Live optimized sync + remote-sleep backend smoke passed; iOS APNs configured | Project `babyrelay-ruvix`; iOS app `com.ruvixlabs.babyrelay`; Android app `com.ruvixlabs.babyrelay` / `1:500197010265:android:cf90d1f6dc5b788a287a48`; Web app `1:500197010265:web:d6c4297117240e90287a48`; Firestore `(default)` in `nam5`; plist and `google-services.json` are bundled in the platform targets. Anonymous Auth is enabled. Firestore rules are deployed to the `cloud.firestore` release as ruleset `31cc5f6a-3b80-4f03-89f1-45363b801e82` using the Ruvix gcloud context and Firebase Rules REST API. Cloud Function `onSleepEventWritten` is deployed as Gen 2 Node.js 22 in `us-central1`, revision `onsleepeventwritten-00002-cay`, with Eventarc trigger in `nam5`. A client-side owner/joiner invite-code smoke passed on 2026-06-22; a 2026-07-07 remote-sleep backend smoke passed with two fresh anonymous users, invite join, private device-token rule denial, owner sleep write, successful Function request (`200`), and synthetic doc cleanup readback. On 2026-07-09, the Ruvix APNs auth key was capability-proved and uploaded to both the development and production FCM slots for the exact BabyRelay iOS app. |
...
| App Store Connect | App record + metadata/screenshots/categories/age rating/subscriptions/privacy/review/availability ready; no build yet | App ID `6779147183`, name `BabyRelay: Shared Baby Care`, SKU `BabyRelay`; editable version `1.0` is in `PREPARE_FOR_SUBMISSION`; en-US app info/version metadata and six `1320x2868` screenshots are pushed and read back `COMPLETE`. Primary category is `HEALTH_AND_FITNESS`, secondary category is `LIFESTYLE`, age rating is `4+`, content rights are `DOES_NOT_USE_THIRD_PARTY_CONTENT`, and privacy nutrition is published. The established Ruvix App Review contact and BabyRelay-specific review notes are saved. Availability is initialized for all 175 territories with new territories enabled. Bundle ID `7PA3RQ369P` has `IN_APP_PURCHASE` and `PUSH_NOTIFICATIONS`; active Ruvix App Store distribution profiles are installed for Runner and the Live Activity extension. RevenueCat server notifications are confirmed for production + sandbox, both `V2`. Subscription group `22150100`; special annual `6779256297` at `$29.99`, monthly `6779156238` at `$9.99`, annual `6779156833` at `$59.99`; annual has a 7-day free trial. All subscriptions remain `MISSING_METADATA` until they travel with the first App Store version/build submission. ASC still has zero uploaded iOS builds, its only current validator blocker. |
...
| App Store assets/metadata | en-US launch metadata is documented in `docs/app-store-metadata.md` and reflected in AppStore Co-Pilot project `irq0wa833wWMRsASUxfK`. Privacy Policy, Terms, Support URL, six screenshots, categories, age rating, content rights, privacy nutrition, review contact/notes, and 175-territory availability are complete. Still needs build upload/selection, first-time subscription attachment, compliance selection, and final submission. | AppStore Co-Pilot / ASC |
...
1. Run a final in-app two-device UI smoke for invite-code entry and remote
   ...
   membership, optimized shared event doc write/read, remote sleep function
   trigger, and cleanup passed live on 2026-07-07.
2. Upload and select the first App Store/TestFlight build. App Review contact,
   review notes, and 175-territory availability are already complete.
