...
| AppRefer | Foundation configured and webhook-proven | App `app_16e4ca28f81`; link `babyrelay-meta`; live/test SDK keys stored in `mc-vault`; SDK enabled. Canonical App Store and Play destinations are configured. RevenueCat integration reads `connected=true`, and the app-scoped RevenueCat webhook test returns `200`. Release-proven attribution still requires a real paid click → TestFlight/store install/open → sandbox/production purchase readback. |
| RevenueCat | Test Store + App Store catalog mapped; AppRefer forwarding enabled | Project `26c4f023`; Test Store app `appf68d685da8`; App Store app `app70e3a91be4`; App Store SDK key stored in `mc-vault`; Ruvix in-app purchase key and ASC API key are configured. All three App Store products are attached to `pro` and their intended offerings. App-scoped webhook `whintgr70a141be8d` sends all production and sandbox events to BabyRelay's AppRefer endpoint and returned `200` in the dashboard test. Apple server notification URLs are confirmed set in ASC for production + sandbox, both `V2`. Google Play products exist, but the RevenueCat Android app, product import, and public SDK key still need to be created from the BabyRelay-specific RevenueCat account. Account email confirmation remains a non-release support cleanup. |
...
| AppRefer paid-path proof | Real paid click and physical/TestFlight install/open/purchase | Store destinations, SDK key injection, RevenueCat identity bridge, and webhook are configured; read back the complete click → install/open → purchase join before paid acquisition. |
