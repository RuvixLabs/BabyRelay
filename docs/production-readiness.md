...
| Google Play | Android target ready; Play shell missing | Android package `com.ruvixlabs.babyrelay` builds and launches on device with Firebase config. Ruvix Play service-account auth works, but source-of-truth readback returns `Package not found: com.ruvixlabs.babyrelay`, so the Play Console app shell, listing, app-content forms, upload signing, subscriptions/base plans/offers, RTDN, and RevenueCat Android app are still pending. |
...
| Android Play shell | Ruvix Play Console app for `com.ruvixlabs.babyrelay` | Unblocks Play listing, upload signing, Google Play subscriptions, RevenueCat Android app, and RTDN |
| Android RevenueCat / Play products | Play subscriptions/base plans/offers + RevenueCat Android SDK key | Supply `REVENUECAT_ANDROID_API_KEY` and validate Android package fetching, purchase, restore, and entitlement `pro` |
