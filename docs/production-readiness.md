...
| Universal/deep links | The Next.js fallback is deployed on Firebase App Hosting. `/.well-known/apple-app-site-association` limits iOS handling to `/join/*`; `/join/<code>` validates the code and forwards first installs through AppRefer with `invite_code=<code>`; `/.well-known/assetlinks.json` fails closed until `ANDROID_SHA256_CERT_FINGERPRINTS` contains the Play App Signing SHA-256 value. Register and attach `ourbabyrelay.com`, then verify direct HTTP `200` responses without a `www` redirect. | Join links open the installed app directly; first installs recover the prefilled invite after the store handoff |
