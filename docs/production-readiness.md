...
| App Store Connect | Current build valid, attached, and internally testing; web-only declarations remain | App ID `6779147183`, name `BabyRelay: Shared Baby Care`, SKU `BabyRelay`; editable version `1.0` is in `PREPARE_FOR_SUBMISSION`. Signed build `1.0 (6)` (`03df36e2-a0f5-458a-a042-04bf4dfe7912`) is Apple `VALID`, attached to version 1.0, and the only build in `BabyRelay Internal`; TestFlight validation is clean. Stale build 3 was detached from the version and removed from the internal group. Metadata, six `1320x2868` screenshots, categories, `4+` age rating, content rights, published privacy nutrition, review contact/notes, free app price, and 175-territory availability are complete. App Review notes now name Superwall and build 6. All three subscriptions are `READY_TO_SUBMIT` and priced in all 175 territories: special annual `$29.99`, monthly `$9.99`, annual `$59.99` with a 7-day trial. Add for Review is narrowed to the web-only regulated-medical-device declaration; the three first-time subscriptions must then be selected on version 1.0. Final submission remains intentionally held for physical push/Live Activity and sandbox purchase/restore proof. |
| Google Play | Current signed build 6 published to Internal testing; RTDN proven | Ruvix Play app `4973846096696226350`, package `com.ruvixlabs.babyrelay`, title `BabyRelay: Shared Baby Care`. Signed `1.0.0 (6)` is the completed Internal-track release with Ruvix-owned upload signing and Play App Signing active; its AAB SHA-256 is `f82dd5ad283a38df46523105fa6b0a3543e9d95d3a5c519b21cdd3be38d81d42`. Build 6 includes the production entitlement, Firebase fail-closed, shared-alert, and remote Live Activity hardening on top of the build-5 FlutterFire codec correction. Listing metadata, support email, approved glow-face icon, and six shared-care phone screenshots are uploaded. Play subscriptions are active: `babyrelay_pro_monthly` (`monthly`, `$9.99`, no trial), `babyrelay_pro_annual` (`annual`, `$59.99`, 7-day trial), and `babyrelay_pro_special_annual` (`special-annual`, `$29.99`, no trial), all across `173` regions. RTDN is saved to `projects/babyrelay-ruvix/topics/babyrelay-google-play-rtdn` with all purchase notification types; Superwall's app-owned subscription and a live test webhook are proven. The Advertising ID declaration is saved as Yes for Analytics and Advertising or marketing and is intentionally held in Publishing overview with the production packet. A Play-installed physical purchase/restore QA pass remains. |
...
2. Reauthenticate the registered Ruvix ASC profile, set regulated medical
   device to `No`, update the three subscription review notes from build 3 to
   build 6, and select the three first-time subscriptions on version 1.0.
   Build `1.0 (6)` is already valid, attached, and the only build in internal
   testing.
