...
| Universal/deep links | Platform declarations and a deploy-ready Vercel web package are committed. `website/.well-known/apple-app-site-association` limits iOS handling to `/join/*`; `/join/<code>` validates the code and forwards first installs through AppRefer with `invite_code=<code>`; `api/assetlinks.js` fails closed until `ANDROID_SHA256_CERT_FINGERPRINTS` contains the Play App Signing SHA-256 value. The live Vercel project still needs this package deployed without an apex-to-`www` redirect on `/.well-known/*`. | Join links open the installed app directly; first installs recover the prefilled invite after the store handoff |
