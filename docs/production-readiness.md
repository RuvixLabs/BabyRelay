...
| Remote sleep push delivery | Upload APNs Auth Key/capabilities in Apple/Firebase; TestFlight/physical iPhone and Android push smoke with real device tokens | Backend deploy and rules/function smoke are done; remaining proof is real owner sleep start/end waking caregiver lock-screen Live Activity and Android ongoing notification without using the source device |
